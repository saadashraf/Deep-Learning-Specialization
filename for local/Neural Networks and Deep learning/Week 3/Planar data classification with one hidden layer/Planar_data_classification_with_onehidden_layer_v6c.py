...
def compute_cost(A2, Y, parameters):
    ...
    logprobs = np.dot(np.log(A2) , Y.T) + np.dot((1 - Y) , np.log(1 - A2).T)
    cost = -(logprobs / m)
    ### END CODE HERE ###
...
plt.subplot(4 , 1, 2)
plt.axis((-3 , 3 , -2 , 2))
parameters = nn_model(X, Y, 10, num_iterations = 10000)
plot_decision_boundary(lambda x: predict(parameters, x.T), X, Y)
predictions = predict(parameters, X)
accuracy = float((np.dot(Y,predictions.T) + np.dot(1-Y,1-predictions.T))/float(Y.size)*100)
print ("Accuracy for {} hidden units: {} %".format(10, accuracy))
