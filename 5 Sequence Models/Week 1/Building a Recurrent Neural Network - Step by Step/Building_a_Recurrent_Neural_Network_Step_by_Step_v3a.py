...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dz = ot * (1 - np.square(np.tanh(c_next))) * da_next
    dot = da_next * np.tanh(c_next) * ot * (1 - ot)
    dcct = ((dc_next * it) + (dz * it)) * (1 - np.square(cct))
    dit = ((dc_next * cct) + (dz * cct)) * it * (1 - it)
    dft = ((dc_next * c_prev) + (dz * c_prev)) * ft * (1 - ft)
    
    # Compute parameters related derivatives. Use equations (11)-(18) (≈8 lines)
    ax = np.concatenate((a_prev , xt) , axis = 0)
    dWf = np.dot(dft , np.transpose(ax))
    dWi = np.dot(dit , np.transpose(ax))
    dWc = np.dot(dcct , np.transpose(ax))
    dWo = np.dot(dot , np.transpose(ax))
