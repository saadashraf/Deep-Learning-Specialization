...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = ((e_w1B - mu_B) * (np.sqrt(np.absolute(1 - np.sum(np.square(mu_orth)))))) / np.sqrt(np.sum(np.square((e_w1 - mu_orth) - mu_B)))
    corrected_e_w2B = ((e_w2B - mu_B) * (np.sqrt(np.absolute(1 - np.sum(np.square(mu_orth)))))) / np.sqrt(np.sum(np.square((e_w2 - mu_orth) - mu_B)))
